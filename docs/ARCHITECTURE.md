...
- **`routes/`**: Flask blueprints organized by feature (**23** registered in `app.py`)
  ...
  - `routes/epg/` - EPG sources, channels, match rules, Schedules Direct, XMLTV grabbers
  - `routes/ppv_enrichment.py`, `ppv_epg.py` - PPV enrichment and event EPG
...
### Data Layer
- **`models/`**: SQLAlchemy models split by domain (`__init__.py` re-exports all 44 models)
...
Supported source types: `schedules_direct`, `xmltv_url`, `xmltv_grabber`, and `ppv_events`. The old `provider` passthrough type is removed from the API; existing rows may remain in the database until deleted manually.
