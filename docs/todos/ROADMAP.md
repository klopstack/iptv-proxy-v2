...
1. **[69](./69-lock-down-destructive-admin-endpoints.md)** — remove HTTP FCC DROP TABLE  
2. **[52](./52-fix-details-fetched-stat.md) + [54](./54-route-enrichment-through-persist-match.md)** — PPV metrics/persist (PR C)  
3. **[75](./75-fix-side-effect-get-account-categories.md)** — idempotent GET categories  
4. **[55](./55-multi-source-events-schema-and-detail-fetch.md)** — plan migration window (PR D)  
5. **[91](./91-scheduler-status-api-failure-metadata.md)** — scheduler failure visibility (PR K)  
