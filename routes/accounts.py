...
def preview_account_playlist(account_id):
    ...
    has_synced = db.session.query(Channel.id).filter(
    ...
    if not has_synced:
        return jsonify({
            "success": False, 
            "error": "Account not synced. Please sync the account first."
        }), 503

    # Build base query - use pre-computed is_visible
    base_query = db.session.query(Channel).filter(
        Channel.account_id == account_id,
        Channel.is_active == True,
        Channel.is_visible == True  # Use pre-computed filter result
    ).join(Category, Channel.category_id == Category.id, isouter=True).order_by(Channel.name)
