...
def _sofascore_football_enabled() -> bool:
    return Settings.get(SETTING_PPV_SOFASCORE_FOOTBALL_ENABLED, "true").lower() in (
        "true",
        "1",
        "yes",
        "on",
...
def update_ppv_enrichment_config():
    ...
    if "sofascore_football_enabled" in data:
        Settings.set(
            SETTING_PPV_SOFASCORE_FOOTBALL_ENABLED,
            "true" if data["sofascore_football_enabled"] else "false",
        )
