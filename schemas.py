...
class FilterCreateSchema(Schema):
    ...
    filter_type = fields.Str(
        required=True,
        validate=lambda x: x in ['category', 'channel_name', 'regex', 'tag']
    )
    filter_action = fields.Str(
        required=True,
        validate=lambda x: x in ['whitelist', 'blacklist']
    )
...
class TagRuleCreateSchema(Schema):
    ...
    pattern_type = fields.Str(
        required=True,
        validate=lambda x: x in ['prefix', 'suffix', 'contains', 'regex']
    )
    tag_name = fields.Str(required=True, validate=lambda x: 1 <= len(x) <= 100)
    source = fields.Str(
        required=True,
        validate=lambda x: x in ['channel_name', 'category_name', 'both']
    )
...
class PlaylistConfigCreateSchema(Schema):
    ...
    def validate_accounts(self, data, **kwargs):
        ...
        if overlap:
            raise ValidationError(
                f"Accounts cannot be in both include and exclude: {overlap}",
                field_name='include_accounts'
            )
    ...
    def validate_tags(self, data, **kwargs):
        ...
        if overlap:
            raise ValidationError(
                f"Tags cannot be in both include and exclude: {overlap}",
