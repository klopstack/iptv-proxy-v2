...
Extracted from routes/config_transfer.py so handlers stay thin (parse → service → respond).
"""
...
def _empty_import_stats() -> dict[str, dict[str, int]]:
    return {
...
def _resolve_account(
    name: Optional[str],
    create_missing: bool,
    account_payload_by_name: dict[str, dict],
) -> Optional[Account]:
    if not name:
        return None
    ...
    account = Account(
        ...
        user_agent=payload.get("user_agent") or DEFAULT_USER_AGENT,
...
class ConfigTransferService:
    """Export and import portable configuration bundles."""

    @staticmethod
    def export_bundle(*, include_accounts: bool = True) -> dict[str, Any]:
        """Build a full configuration bundle dict from the database."""
        ...
        return {
            ...
            "accounts": [_serialize_account(account) for account in accounts] if include_accounts else [],
            "filters": [
                {
    ...
    def import_bundle(
        ...
        *,
        ...
        """Apply a validated configuration bundle and return import stats."""
        ...
        ConfigTransferService._import_accounts(accounts_data, overwrite, stats)
        db.session.flush()
        ...
        epg_rulesets_by_name = ConfigTransferService._import_epg_match_rulesets(data, overwrite, stats)
        ConfigTransferService._import_account_epg_match_ruleset_assignments(
            data, overwrite, create_missing_accounts, account_payload_by_name, epg_rulesets_by_name, stats
        )
        ConfigTransferService._import_epg_exclusion_patterns(data, overwrite, stats)
        ConfigTransferService._import_epg_channel_name_mappings(data, overwrite, stats)
        ConfigTransferService._import_fcc_patterns(data, overwrite, stats)
    ...
    @staticmethod
    def _import_accounts(accounts_data: list[dict], overwrite: bool, stats: dict) -> None:
        for account_data in accounts_data:
            ...
            db.session.add(
                Account(
                    name=name,
                    server=server,
                    user_agent=account_data.get("user_agent") or DEFAULT_USER_AGENT,
    ...
    @staticmethod
    def _import_rulesets(data: dict, overwrite: bool, stats: dict) -> dict[str, RuleSet]:
    ...
    @staticmethod
    def _import_account_ruleset_assignments(
        data: dict,
        overwrite: bool,
        create_missing_accounts: bool,
        account_payload_by_name: dict[str, dict],
    ...
    @staticmethod
    def _import_epg_match_rulesets(data: dict, overwrite: bool, stats: dict) -> dict[str, EpgMatchRuleSet]:
            ...
            for rule_data in ruleset_data.get("rules") or []:
                ...
                db.session.add(
                    EpgMatchRule(
                        ...
                        required_tags=json.dumps(rule_data["required_tags"])
                        if rule_data.get("required_tags")
                        else None,
                        excluded_tags=json.dumps(rule_data["excluded_tags"])
                        if rule_data.get("excluded_tags")
                        else None,
                        ...
                        country_codes=json.dumps(rule_data["country_codes"])
                        if rule_data.get("country_codes")
                        else None,
                        epg_source_ids=json.dumps(rule_data["epg_source_ids"])
                        if rule_data.get("epg_source_ids")
                        else None,
        ...
        return epg_rulesets_by_name

    @staticmethod
    def _import_account_epg_match_ruleset_assignments(
        data: dict,
        overwrite: bool,
        create_missing_accounts: bool,
        account_payload_by_name: dict[str, dict],
    ...
    @staticmethod
    def _import_epg_exclusion_patterns(data: dict, overwrite: bool, stats: dict) -> None:
    ...
    @staticmethod
    def _import_epg_channel_name_mappings(data: dict, overwrite: bool, stats: dict) -> None:
    ...
    @staticmethod
    def _import_fcc_patterns(data: dict, overwrite: bool, stats: dict) -> None:
