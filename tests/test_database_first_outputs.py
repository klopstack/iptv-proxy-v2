"""
Tests for database-first enforcement on tag processing, preview, and playlist output.
"""
import pytest
...
@pytest.fixture
def database_first_account(app):
    """Create test account for database-first output tests."""
    with app.app_context():
        account = Account(
            name="Database First Test Account",
            server="test.example.com",
            username="dbfirstuser",
            password="dbfirstpass",
            enabled=True,
        )
...
@pytest.fixture
def synced_account_with_channels(app, database_first_account):
    """Create account with synced channels"""
    with app.app_context():
        # Create category
        category = Category(account_id=database_first_account.id, category_id="100", category_name="Test Category")
        ...
        channels = [
            Channel(
                account_id=database_first_account.id,
                stream_id=f"ch{i}",
...
def test_tag_processing_returns_503_when_not_synced(app, client, database_first_account):
    """Test that tag processing requires synced channels"""
    with app.app_context():
        # No channels synced for this account
        channel_count = Channel.query.filter_by(account_id=database_first_account.id, is_active=True).count()
        assert channel_count == 0

        # Try to process tags
        response = client.post(f"/api/accounts/{database_first_account.id}/process-tags")
...
def test_tag_processing_works_when_synced(app, client, synced_account_with_channels):
    ...
    with app.app_context():
        # Channels exist
        channel_count = Channel.query.filter_by(account_id=synced_account_with_channels.id, is_active=True).count()
        assert channel_count > 0
...
def test_preview_returns_503_when_not_synced(app, client, database_first_account):
    ...
    with app.app_context():
        ...
        # Try to preview
        response = client.get(f"/api/accounts/{database_first_account.id}/preview")
...
def test_preview_with_pagination(app, client, synced_account_with_channels):
    ...
    with app.app_context():
        # Request page 1
        response = client.get(f"/api/accounts/{synced_account_with_channels.id}/preview?limit=2&offset=0")
        ...
        # Request page 2
        response2 = client.get(f"/api/accounts/{synced_account_with_channels.id}/preview?limit=2&offset=2")
...
def test_playlist_returns_503_when_not_synced(app, client, database_first_account):
    ...
    with app.app_context():
        ...
        response = client.get(f"/playlist/{database_first_account.id}.m3u")
...
def test_no_api_fallback_in_tag_processing(app, client, database_first_account):
...
def test_no_api_fallback_in_preview(app, client, database_first_account):
...
def test_database_first_workflow(app, client, database_first_account):
    """Test the enforced database-first workflow"""
    with app.app_context():
        # Step 1: Account exists but not synced - operations should fail

        # Tag processing fails
        tag_response = client.post(f"/api/accounts/{database_first_account.id}/process-tags")
        assert tag_response.status_code == 503

        # Preview fails
        preview_response = client.get(f"/api/accounts/{database_first_account.id}/preview")
        assert preview_response.status_code == 503

        # Playlist fails
        playlist_response = client.get(f"/playlist/{database_first_account.id}.m3u")
        assert playlist_response.status_code == 503

        # Step 2: Create synced channels (simulating sync)
        category = Category(account_id=database_first_account.id, category_id="200", category_name="Synced Category")
        db.session.add(category)
        db.session.flush()
        ...
        tag_response2 = client.post(f"/api/accounts/{database_first_account.id}/process-tags")
        assert tag_response2.status_code == 200

        # Preview works
        preview_response2 = client.get(f"/api/accounts/{database_first_account.id}/preview")
        assert preview_response2.status_code == 200

        # Playlist works
        playlist_response2 = client.get(f"/playlist/{database_first_account.id}.m3u")
        assert playlist_response2.status_code == 200


def test_error_messages_are_clear(app, client, database_first_account):
...
def test_inactive_channels_ignored(app, client, database_first_account):
    """Test that only active channels are considered for sync status"""
    with app.app_context():
        # Create inactive channel
        category = Category(account_id=database_first_account.id, category_id="300", category_name="Test")
        db.session.add(category)
        db.session.flush()
