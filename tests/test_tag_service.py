...
class TestEdgeCases:
    ...
    def test_extract_tags_empty_channel_name(self, app):
        ...
        with app.app_context():
            ...
            tags, cleaned_name = TagService.extract_tags("", "Sports", [rule])
            assert len(tags) == 0
            assert cleaned_name == ""
    ...
    def test_extract_tags_empty_category_name(self, app):
        ...
        with app.app_context():
            ...
            tags, cleaned_name = TagService.extract_tags("ESPN", "", [rule])
    ...
    def test_extract_tags_no_rules(self, app):
        ...
        with app.app_context():
            tags, cleaned_name = TagService.extract_tags("US| ESPN 4K", "Sports", [])
            assert len(tags) == 0
    ...
    def test_extract_tags_invalid_regex_pattern(self, app):
        ...
        with app.app_context():
            ...
            tags, cleaned_name = TagService.extract_tags("ESPN", "Sports", [rule])
            assert len(tags) == 0
    ...
    def test_extract_tags_priority_order(self, app):
        ...
        with app.app_context():
            ...
            tags, cleaned_name = TagService.extract_tags("ESPN 4K", "Sports", [rule_high, rule_low])
    ...
    def test_extract_tags_with_unicode_channel_name(self, app):
        ...
        with app.app_context():
            ...
            tags, cleaned_name = TagService.extract_tags(channel_name, "Sports", [rule])
...
class TestExtractTagsIntegration:
    ...
    def test_extract_tags_with_location_and_callsign(self, app):
        ...
        with app.app_context():
            ...
            tags, cleaned_name = TagService.extract_tags(channel_name, "News", rules)
